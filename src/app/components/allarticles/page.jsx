...
const AllArticles = () => {
  ...
  const handleOpenModal1 = () => setOpen2(true);
  const handleCloseModal1 = () => setOpen2(false);
...
if(useruid1==""|| title==""|| discription==""||url==""){
  ...
  const [tittle1,settitle1]=useState("")
...
seteditid(e)
  ...
  const handleEditArticle = ()=>{
    ...
    let config = {
      method: 'put',
      maxBodyLength: Infinity,
      url: `http://localhost:3000/api/userEdit/${editid}`,
      headers: { },
      data : {
        title:tittle1,
        discription:dis,
...

    {
      ...
      <CardContent>
        ...
        <Box sx={{display:'flex',justifyContent:'center'}}>
          ...
          <Button variant='contained' sx={{backgroundColor:'blue !important'}} onClick={()=>handlseEdit(v._id)}>Edit</Button>
        </Box>
        }
      </CardContent>
      ...
      >
        ...
        <Typography id="modal-modal-title" variant="h6" component="h2">
          ...
          <TextField id="standard-basic"value={tittle1} onChange={(e)=>settitle1(e.target.value)} fullWidth label="Title" variant="standard" required />
          <TextField id="standard-basic" value={dis} onChange={(e)=>setdis(e.target.value)} sx={{mt:2}} fullWidth label="Discription" variant="standard" required/>
          <TextField id="standard-basic" value={img} sx={{mt:2}} onChange={(e)=>setimg(e.target.value)} fullWidth label="Image" variant="standard" required/>
          <Box sx={{display:'flex',justifyContent:'center',mt:2}}>
          <Button color='error' sx={{mr:2}} variant='outlined' onClick={handleClose}>
          close
      </Button>
          <Button color='primary' sx={{backgroundColor:'blue !important'}} variant='contained' onClick={handleEditArticle}>
          Save
      </Button>
